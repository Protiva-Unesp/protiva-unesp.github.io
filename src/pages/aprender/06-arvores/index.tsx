                        ...
                        {data.allMarkdownRemark.nodes.map((node, index) => (
                            <div key={index} className="w-full sm:w-1/2 lg:w-1/3 px-4 mb-8 flex">
                                <div className="flex flex-col flex-grow">
                                    <Link to={`/aprender/06-arvores/${node.frontmatter.slug}`} className="flex flex-col flex-grow">
...
export const query = graphql`
    query {
        allMarkdownRemark(
            filter: { fileAbsolutePath: { regex: "/06-arvores/" } }
...
export const Head = () => <Seo title="Árvores"/>
